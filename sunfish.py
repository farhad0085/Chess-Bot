...
startEngine = ObjectProperty(None)

class RelativeLayout(RelativeLayout):  
    def btn(self):
        status = self.startEngine.text
        self.startEngine.text = "Engine Started"
...
def main():
    ...
    while True:
        ...
        if hist[-1].score <= -MATE_LOWER:
            printComputerMove("YOULOST")
            print("You Lost") # when user lost
            break
        ...
        if hist[-1].score <= -MATE_LOWER:
            printComputerMove("YOUWON") # when user won
            print("You won") 
            break
        ...
        if score == MATE_UPPER:
            #printComputerMove("CHECKMATE") # CHECKMATE
            print("Checkmate!")

        # The black player moves from a rotated position, so we have to
        # 'back rotate' the move before printing it.
        computerMoves = render(119-move[0]) + render(119-move[1])
        printComputerMove(computerMoves)
        i += 1
        hist.append(hist[-1].move(move))
